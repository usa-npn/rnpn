...
`npn_export_individual_phenometrics()` requests the same data product through
the newer asynchronous export service, which is much better at large requests.
It is experimental and sits alongside `npn_download_individual_phenometrics()`
rather than replacing it; see `vignette("IX_async_exports")`.
...
`start_date` and `end_date` are required, and they are **not** one continuous
span. The service decomposes the range into one window per phenological year,
anchored on the **month-day** of the two dates:
...
This is what the legacy function's `period_start` / `period_end` arguments did,
arriving through the date arguments instead of through two more of them. A
range whose `end_date` falls no later in the year than its `start_date` wraps
the turn of the year and must span at least a full year, or it contains no
complete window and is an error.

`npn_export_magnitude_phenometrics()` is never windowed --- do not carry this
conclusion across to it.
