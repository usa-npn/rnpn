test_that("npn_stations functions", {
  ...
  vcr::local_cassette("npn_stations")
...
test_that("npn_stations_by_state functions", {
  skip_if_not(check_service(), "Service is down")
  vcr::local_cassette("npn_stations_by_state")
...
test_that("npn_stations_by_location functions", {
  skip_if_not(check_service(), "Service is down")
  vcr::local_cassette("npn_stations_by_location")

  stations <- npn_stations_by_location(
    wkt = "POLYGON((-110.94484396954107 32.23623109416672,-110.96166678448247 32.23594069208043,-110.95960684795904 32.21328646993733,-110.94244071026372 32.21343170728929,-110.93935080547857 32.23216538049456,-110.94484396954107 32.23623109416672))"
  )
  expect_s3_class(stations, "data.frame")
...
test_that("npn_stations_with_spp functions", {
  skip_if_not(check_service(), "Service is down")
  vcr::local_cassette("npn_stations_with_spp")
